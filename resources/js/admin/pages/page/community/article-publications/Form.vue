					...
					<div class="row">
						...
						<div class="col_12">
							<div class="form-group">
								<label>{{
									trans(
										"admin.form.article_publications.content_en"
									)
								}}</label>
								<froala-text-editor
						...
						<div class="col_12">
							<div class="form-group">
								<label>{{
									trans(
										"admin.form.article_publications.content_ko"
									)
								}}</label>
								<froala-text-editor
...
<script>
	...
	export default {
		...
		created() {
			if (this.$route.name === "ics_articles_edit") {
				this.loading = true;
				this.pageName = this.trans(
					"admin.form.article_publications.edit_article_publications"
				);
				axios
					.get("/api/admin/articles-publications/" + this.$route.params.id)
					.then((response) => {
		...
		methods: {
			submitForm() {
				...
				if (this.$route.name === "ics_articles_edit") {
					axios
						.patch(
							"/api/admin/articles-publications/" + this.$route.params.id,
							this.form
						)
						.then((response) => {
							this.$router.push({ name: "ics_articles_list" });
						})
						.catch((err) => (this.errors = err.response.data.errors))
						.finally(() => (this.loading = false));
				} else {
					axios
						.post("/api/admin/articles-publications", this.form)
						.then((response) => {
							this.$router.push({ name: "ics_articles_list" });
						})
						.catch((err) => (this.errors = err.response.data.errors))
