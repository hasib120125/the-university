    ...
    <div>
        <div class="row">
            <div class="col_12">
                <div class="card">
                    <div class="card-body">
                        <router-link :to="{ name: 'lecture_notice_add' }" class="btn btn_info">{{ trans('admin.form.notices.add_new_notice') }}</router-link>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col_12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">{{ trans('admin.form.lecture.notices') }}</div>
                    </div>
                    <div class="card-body">
                        <table-component :api-url="`/api/admin/lectures/${$route.params.id}/notices`"
                                         :fields="fields"
                                         ref="tableComponent"
        ...
        deleteItem(item) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete(`/api/admin/lectures/${item.lecture_id}/notices/${item.id}`).then(() => {
