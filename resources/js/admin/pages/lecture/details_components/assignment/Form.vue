            ...
            <div class="card">
                ...
                <div :class="['card-body', {'loading_overlay': loading}]">
                    ...
                    <div class="d_flex_end">
                        <router-link class="btn btn_secondary mr_5" :to="{name: 'lecture_assignments'}">{{ trans('admin.label.cancel') }}</router-link>
...
export default {
    ...
    created() {
        if (this.$route.name === 'lecture_assignment_update') {
    ...
    methods: {
        ...
        submitForm(){
            ...
            if (this.$route.name === 'lecture_assignment_update') {
                ...
                axios.post('/api/admin/lectures/' + this.$route.params.id + '/assignments/'+ this.$route.params.assignment_id, formData)
                    .then((response) => {
                        this.$router.push({name: 'lecture_assignments'});
                    })
                    .catch((err) => this.errors = err.response.data.errors)
                    .finally(() => this.loading = false);

            }else{
                axios.post('/api/admin/lectures/' + this.$route.params.id + '/assignments', formData)
                    .then((response) => {
                        this.$router.push({name: 'lecture_assignments'});
                    })
                    .catch((err) => this.errors = err.response.data.errors)
