    ...
    <div>
        <div class="row">
            <div class="col_12">
                <div class="card">
                    <div class="card-body">
                        <router-link :to="{ name: 'socials_add' }" class="btn btn_info">{{ trans('admin.form.socials.add_new_socials') }}</router-link>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col_12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">{{ trans('admin.form.socials.socials') }}</div>
                    </div>
                    <div class="card-body">
                        <table-component api-url="/api/admin/socials"
                                         :fields="fields"
                                         ref="tableComponent"
    ...
    data() {
        return {
            fields: [
                {
                    name: 'name',
                    title: this.trans('admin.form.socials.name'),
                    sortField: 'name',
                    searchable: true
                },
                {
                    name: 'url',
                    title: this.trans('admin.form.socials.link'),
                    sortField: 'url',
                    searchable: true
                },
                {
                    name: 'class',
                    title: this.trans('admin.form.socials.font_class'),
                    sortField: 'class',
                    searchable: true
                },
                {
        ...
        deleteItem(item) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete('/api/admin/socials/'+item.id).then(() => {
