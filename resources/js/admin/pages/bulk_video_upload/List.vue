            ...
            <div class="col_12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">{{ trans('admin.form.bulk_upload.video_list') }}</div>
                    </div>
                    <div class="card-body">
                        <button class="btn btn_md btn_success" @click.prevent="checkFtp">Check FTP</button>

                        <table-component api-url="/api/admin/bulk-video"
                                         :fields="fields"
                                         ref="tableComponent"
        ...
        <div class="modal" :class="{'open_modal': open_modal === true}" data-modal="modal">
            ...
            <div class="modal_inner">
                <div class="modal_header">
                    ...
                    <h2>{{ trans('common.index.assign_video_to_lecture') }}</h2>
                </div>
                            ...
                            <div class="row">
                                <div class="col_4">
                                        <div class="form-group ">
                                            ...
                                            <Select2 v-model="form.subject_id" :options="subjectsForSelect2"
                                                     :settings="{ multiple: false, width: '100%' }"
                                                     :placeholder="trans('admin.form.bulk_upload.select_subject')"
                                                     @change="loadLecture" />
...
<!--                                            <select class="form_global" v-model="form.subject_id" @change="loadLecture">
                                ...
                                <div class="col_4">
                                        <div class="form-group ">
                                            ...
                                            <Select2 v-model="form.semester_id" :options="semestersForSelect2"
                                                     :placeholder="trans('admin.form.bulk_upload.select_semester')"
                                                     :settings="{ multiple: false, width: '100%' }"
                                                     @change="loadLecture" />
...
<!--                                            <select class="form_global" v-model="form.semester_id" @change="loadLecture">
                                ...
                                <div class="col_4">
                                        <div class="form-group ">
                                            ...
                                            <Select2 v-model="form.lecture_id" :options="LecturesForSelect2"
                                                     :placeholder="trans('admin.form.bulk_upload.select_lecture')"
                                                     :settings="{ multiple: false, width: '100%' }" />
...
<!--                                            <select class="form_global" v-model="form.lecture_id">
                            ...
                            <div class="d_flex_end">
                                ...
                                <button class="btn btn_md btn_success" @click.prevent="assignVideoToLecture">{{ trans('admin.label.save') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal " :class="{ 'open_modal': showModal}"  data-modal="modal">
            <div class="modal_overlay" data-modal-close="modal" @click="closeVideo"></div>
            <div class="modal_inner">
                <div class="modal_wrapper ">
                    <div class="modal_content modal_1080p">
                        <div id="player"></div>
...
export default {
    ...
    data() {
        return {
            ...
            fields: [
                ...
                {
                    name: 'convert_status',
                    title: this.trans('admin.form.bulk_upload.process_status'),
                    searchable: false,
                ...
                },
                {
                    ...
                    data: [
                        ...
                        {
                            class: 'btn btn_sm btn_warning mr_5',
                            title: '<i class="fas fa-redo"></i>',
                            tooltip: this.trans('admin.label.retry'),
                            action: 'retry',
                            condition: ['fail', 1]
                        },
                        {
                            class: 'btn btn_sm btn_danger mr_5',
                            title: '<i class="fas fa-history"></i>',
    ...
    computed: {
        subjectsForSelect2() {
            return this.subjects.map(i => ({
                id: i.id, text: i.name
            }));
        },
        semestersForSelect2() {
            return this.semesters.map(i => ({
                id: i.id, text: i.name
            }));
        },
        LecturesForSelect2() {
            return this.lectures.map(i => ({
                id: i.id, text: i.name
            }));
        }
    },
    methods: {
        ...
        convertAgain(item) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.post('/api/admin/bulk/video-file/convert-again/'+item.id).then(() => {
                        this.$refs.tableComponent.refresh();
                    });
                }
            });
        },
        assignVideoToLecture(){
            axios.post(`/api/admin/bulk-video/${this.bulkVideoId}/assign-video-to-lecture`, this.form)
                    .then((response) => {
                        this.$refs.tableComponent.refresh();
                        this.$swal.fire(
        ...
        openModal(bulkVideo){
            ...
            this.bulkVideoId = bulkVideo.id
        },
        closeModal() {
            this.open_modal = false
            this.bulkVideoId = null
            this.subjects= []
            this.semesters= []
            this.lectures= []
        },
        ...
        retry(item) {
            this.$swal({
                title: this.trans('admin.label.retry'),
                text: this.trans('admin.label.retry_warning'),
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: this.trans('admin.label.retry'),
                cancelButtonText: this.trans('admin.label.no_cancel'),
                reverseButtons: true
            }).then((result) => {
                if (result.isConfirmed) {
                    axios.post('/api/admin/bulk/video-file/retry/'+item.id).then(() => {
                        this.$refs.tableComponent.refresh();
                    });
                }
            });
        ...
        deleteItem(item) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete('/api/admin/bulk-video/'+item.id).then(() => {
                        this.$refs.tableComponent.refresh();
                        this.$swal.fire(
