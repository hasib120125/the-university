        ...
        <div class="col_12">
            <div class="card">
                <div class="card-header">
                    <div class="col_10">
                        <div class="card-title">{{ exam && exam.subject ? exam.subject.name : '' }} {{ trans('admin.form.exam.details.exam_questions') }}</div>
                    </div>
                    <div class="col_2">
                        <div class="d_flex_end">
                            <router-link class="btn btn_sm btn_secondary mr_5" :to="{name: 'subject_exam_students'}"> {{ trans('common.index.back') }} </router-link>
                        </div>
                    </div>
                </div>
                <div class="card-body" v-if="exam">
                    <table class="table table_bordered table_fixed">
                        <tbody>
                            <tr>
                                <th>{{ trans('admin.form.exam.title') }}</th>
                                ...
                                <div class="card-body">
                                    <div class="d_flex_btwn">
                                        ...
                                        <div class="d_flex_start ">
                                            <span class="mr_10 fw_bold font_16p">{{ trans('admin.form.exam.question.marks') }}</span>
                                            <input type="number" min="0" :max="question.marks" class="form_global width_50p" v-model="questions[index].student_answer.score" :disabled="[1,2].includes(question.question_type)">
                    ...
                    <div class="row" v-show="questions.length > 0">
                        <div class="col_12">
                            <div class="mt_10">
                                <router-link class="btn btn_secondary mr_5" :to="{name: 'subject_exam_students'}">{{ trans('admin.label.cancel') }}</router-link>
...
export default {
    ...
    created() {
        axios.get(`/api/admin/subjects/exams/${this.$route.params.exam_id}/students/${this.$route.params.student_id}`)
            .then((response) => {
    ...
    methods: {
        ...
        submitForm(){
            this.loading = true
            axios.patch(`/api/admin/subjects/exams/${this.$route.params.exam_id}/students/${this.$route.params.student_id}`, {questions: this.questions})
            .then((response) => {
                this.$swal.fire(
