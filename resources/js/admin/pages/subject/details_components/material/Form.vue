            ...
            <div class="card">
                ...
                <div :class="['card-body', {'loading_overlay': loading}]">
                    ...
                    <div class="d_flex_end">
                        <router-link class="btn btn_secondary mr_5" :to="{name: 'subject_materials', params: {semester_id: $route.params.semester_id}}">{{ trans('admin.label.cancel') }}</router-link>
    ...
    methods: {
        submitForm(){
            ...
            if (this.$route.name === 'subject_material_update') {
                ...
                axios.post('/api/admin/subjects/' + this.$route.params.id + '/materials/'+ this.$route.params.material_id, formData)
                    .then((response) => {
                ...
                axios.post('/api/admin/subjects/' + this.$route.params.id + '/materials', formData)
                    .then((response) => {
                        this.$router.push({name: 'subject_materials', params: {semester_id: this.$route.params.semester_id}});
                    })
                    .catch((err) => this.errors = err.response.data.errors)
