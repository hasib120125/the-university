            ...
            <div class="col_12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">{{ trans('admin.form.bulk_upload.subtitle_list') }}</div>
                    </div>
                    <div class="card-body">
                        <!-- <button class="btn btn_md btn_success" @click.prevent="checkFtp">Check FTP</button> -->

                        <table-component api-url="/api/admin/bulk-subtitle"
                                         :fields="fields"
                                         ref="tableComponent"
        ...
        <div class="modal" :class="{'open_modal': open_modal === true}" data-modal="modal">
            ...
            <div class="modal_inner">
                <div class="modal_header">
                    ...
                    <h2>{{ trans('admin.form.bulk_upload.assign_subtitle_to_lecture') }}</h2>
                </div>
                    ...
                    <div class="modal_content modal_1080p">
                        <div class="container">
                            ...
                            <div class="d_flex_end">
                                    ...
                                    <button class="btn btn_md btn_success" @click.prevent="assignSubtitleToLecture">{{ trans('admin.label.save') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                {
                    name: 'label',
                    title: this.trans('admin.form.bulk_upload.subtitle_label'),
                    searchable: true
                },
                {
                    name: 'action-slot',
    ...
    methods: {
        ...
        assignSubtitleToLecture(){
            axios.post(`/api/admin/bulk-subtitle/${this.bulkSubtitleId}/assign-subtitle-to-lecture`, this.form)
                    .then((response) => {
        ...
        openModal(bulkSubtitle){
            ...
            this.bulkSubtitleId = bulkSubtitle.id
        },
        closeModal(){
            this.open_modal = false
            this.bulkSubtitleId = null
            this.subjects= []
            this.semesters= []
            this.lectures= []
        },
        deleteItem(item) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete('/api/admin/bulk-subtitle/'+item.id).then(() => {
                        this.$refs.tableComponent.refresh();
                        this.$swal.fire(
