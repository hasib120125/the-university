        ...
        <div class="col_12">
            <div class="card">
                ...
                <div :class="['card-body', {'loading_overlay': loading}]">
                    <div class="row">
                        <div class="col_12">
                            <div class="form-group mb_5 mt_15 "><label class="pb_0">{{ trans('admin.form.bulk_upload.subtitle_label') }}</label> </div>
                            <div class="form_radio">
                                <input type="radio" id="label_korean" name="radio" v-model="form.label" value="korean">
                                <label for="label_korean">{{ trans('admin.form.bulk_upload.korean') }}</label>
                            </div>
                            <div class="form_radio">
                                <input type="radio" id="label_english" name="radio" v-model="form.label" value="english">
                                <label for="label_english">{{ trans('admin.form.bulk_upload.english') }}</label>
                            </div>
                        </div>
                        <div class="col_12">
                            <div class="form-group">
                                <label>{{ trans('admin.form.bulk_upload.subtitle') }}</label>
                                <div class="d_flex_inline">
                                    <vue-dropzone 
                                        ref="fileDropzone" 
                                        class="form_global mr_5" 
                                        v-on:vdropzone-removed-file="cancelFile" 
                                        v-on:vdropzone-success="uploadFileSuccess" 
                                        id="dropzone" 
    ...
    methods: {
        ...
        cancelFile(file, error, xhr){
            ...
            if(this.$refs.fileDropzone.dropzone.disabled !== true){
                axios.post('/api/admin/bulk/subtitle-file/delete', {deleteFile}).then((response) => {
                    this.$swal.fire(
        ...
        submitForm(){
            ...
            axios.post('/api/admin/bulk/subtitle-file/upload', this.form)
                .then((response) => {
                    this.$router.push({name: 'bulk_subtitle_list'});
                })
