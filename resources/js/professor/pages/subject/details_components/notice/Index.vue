    ...
    <div>
        <div class="row">
            <div class="col_12">
                <div class="card">
                    <div class="card-body">
                        <router-link :to="{ name: 'subject_notice_add' }" class="btn btn_info">{{ trans('professor.form.notices.add_new_notice') }}</router-link>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col_12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">{{ trans('professor.form.subject.notices') }}</div>
                    </div>
                    <div class="card-body">
                        <table-component :api-url="`/api/professor/subjects/${$route.params.id}/notices?semester_id=${$route.params.semester_id}`"
                                         :fields="fields"
                                         ref="tableComponent"
        ...
        deleteItem(item) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete(`/api/professor/subjects/${item.subject_id}/notices/${item.id}`).then(() => {
