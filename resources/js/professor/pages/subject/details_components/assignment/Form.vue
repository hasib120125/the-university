            ...
            <div class="card">
                ...
                <div :class="['card-body', {'loading_overlay': loading}]">
                    <div class="row">
                        <div class="col_12">
                            <div class="form-group ">
                                <label>{{ trans('professor.form.assignment.title_of_assignment') }}</label>
                    ...
                    <div class="row">
                        <div class="col_3">
                            <div class="form-group ">
                                <label>{{ trans('professor.form.assignment.start_date') }}</label>
                        ...
                        <div class="col_3">
                            <div class="form-group ">
                                <label>{{ trans('professor.form.assignment.end_date') }}</label>
                        ...
                        <div class="col_3">
                            <div class="form-group ">
                                <label>{{ trans('professor.form.assignment.task_type') }}</label>
                                <select class="form_global" id="task_type" v-model="form.task_type">
                                    <option value="1">{{ trans('professor.form.assignment.general_tasks') }}</option>
                                    <option value="2">{{ trans('professor.form.assignment.assignment_type_test') }}</option>
                                    <option value="3">{{ trans('professor.form.assignment.etc') }}</option>
                                </select>
                        ...
                        <div class="col_3">
                            <div class="form-group ">
                                <label>{{ trans('professor.form.assignment.task_type') }} <span class="color_danger">{{ trans('professor.form.assignment.task_type_after') }}</span></label>
                                <select class="form_global" id="end_open" v-model="form.end_open">
                                    <option value="1">{{ trans('professor.form.assignment.submission_allowed') }}</option>
                                    <option value="2">{{ trans('professor.form.assignment.prohibited_to_submit') }}</option>
                                </select>
                    ...
                    <div class="row">
                        <div class="col_12">
                            <div class="form-group ">
                                <label>{{ trans('professor.form.assignment.description') }}</label>
                        ...
                        <div class="col_6">
                            <div class="form-group">
                                <label>{{ trans('professor.form.assignment.attachment_1') }}</label>
                        ...
                        <div class="col_6">
                            <div class="form-group">
                                <label>{{ trans('professor.form.assignment.attachment_2') }}</label>
                                <div class="file_input">
                    ...
                    <div class="d_flex_end">
                        <router-link class="btn btn_secondary mr_5" :to="{name: 'subject_assignments', params: {semester_id: $route.params.semester_id}}">{{ trans('professor.label.cancel') }}</router-link>
...
export default {
    ...
    data(){
        return {
            ...
            loading: false,
            pageName: this.trans('professor.form.assignment.add_new_assignment'),

        }
    },
    created() {
        if (this.$route.name === 'subject_assignment_update') {
            this.pageName = this.trans('professor.form.assignment.edit_assignments');
            this.loading = true;
    ...
    methods: {
        ...
        submitForm(){
            ...
            if (this.$route.name === 'subject_assignment_update') {
                ...
                axios.post('/api/professor/subjects/' + this.$route.params.id + '/assignments/'+ this.$route.params.assignment_id, formData)
                    .then((response) => {
                ...
                axios.post('/api/professor/subjects/' + this.$route.params.id + '/assignments', formData)
                    .then((response) => {
                        this.$router.push({name: 'subject_assignments', params: {semester_id: this.$route.params.semester_id}});
                    })
                    .catch((err) => this.errors = err.response.data.errors)
