    ...
    <div>
        <div class="row">
            <div class="col_12">
                <div class="card">
                    <div class="card-body">
                        <router-link :to="{ name: 'subject_assignment_add' }" class="btn btn_info">{{ trans('professor.form.assignment.add_new_assignment') }}</router-link>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col_12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">{{ trans('professor.form.assignment.assignments') }}</div>
                    </div>
                    <div class="card-body">
                        <table-component :api-url="`/api/professor/subjects/${$route.params.id}/assignments?semester_id=${$route.params.semester_id}`"
                                         :fields="fields"
                                         ref="tableComponent"
            ...
            fields: [
                {
                    ...
                    title: this.trans('professor.form.assignment.title'),
                    sortField: 'assignment_title',
                    searchable: true
                },
                {
                    name: 'start_period',
                    title: this.trans('professor.form.assignment.start_period'),
                    sortField: 'start_period',
                    searchable: true
                },
                {
                    name: 'end_period',
                    title: this.trans('professor.form.assignment.end_period'),
        ...
        deleteItem(item) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete(`/api/professor/lectures/${item.lecture_id}/assignments/${item.id}`).then(() => {
