...
class SubjectExamController extends Controller
{
    ...
    public function store(Request $request, Subject $subject)
    {
        $data = $request->validate(
            ...
            [
                ...
                'questions.*.marks.required' => 'Marks field is required',
                'questions.*.marks.integer' => 'Marks field must be integer',
    ...
    public function examAnswerStore(Request $request, SubjectExam $exam, Student $student)
    {
        ...
        foreach ($questions as $question) {
            if ($question['student_answer']) {
                $score = 0;
                if ($question['student_answer']['correct']) {
                    if (in_array($question['question_type'], [1, 2])) {
                        $score = $question['marks'];
                    }
                    if (in_array($question['question_type'], [3, 4])) {
                        $score = $question['student_answer']['score'];
                    }
                }
    ...
    public function clone(Request $request, SubjectExam $subjectExam, Semester $semester)
    {
        DB::beginTransaction();
        try {
            $cloneSubjectExam = $subjectExam->replicate();
            $cloneSubjectExam->semester_id = $semester->id;
            $cloneSubjectExam->status = false;
            $cloneSubjectExam->save();

            $questions = ExamQuestion::where('subject_exam_id', $subjectExam->id)->get();
            if ($questions->count()) {
                $questionsData = [];
                foreach ($questions as $question) {
                    $questionsData[] = $question->replicate()->toArray();
                }

                $cloneSubjectExam->questions()->createMany($questionsData);
            }
        ...
        return response()->json(['success' => true, 'message' => 'Cloned Successfully!']);
    }
}
